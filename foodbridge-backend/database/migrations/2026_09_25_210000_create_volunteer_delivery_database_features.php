...
return new class extends Migration
{
    public function up(): void
    {
        if (DB::getDriverName() !== 'mysql') {
            $this->createSqliteTestView();
            return;
        }

        DB::statement($this->viewSql());

        DB::unprepared('DROP PROCEDURE IF EXISTS update_volunteer_delivery_status');
        DB::unprepared($this->procedureSql());

        DB::unprepared('DROP TRIGGER IF EXISTS log_volunteer_delivery_status_change');
        DB::unprepared($this->triggerSql());
    }
...
SELECT
    ...
    (
        SELECT r.full_name FROM recipients r
        WHERE r.ngo_id = n.id ORDER BY r.id ASC LIMIT 1
    ) AS recipient_name,
    (
        SELECT r.phone FROM recipients r
        WHERE r.ngo_id = n.id ORDER BY r.id ASC LIMIT 1
    ) AS recipient_phone,
    (
        SELECT r.address FROM recipients r
        WHERE r.ngo_id = n.id ORDER BY r.id ASC LIMIT 1
    ) AS destination_address,
    (
        SELECT r.household_size FROM recipients r
        WHERE r.ngo_id = n.id ORDER BY r.id ASC LIMIT 1
    ) AS household_size,
    d.pickup_time,
    d.delivery_status,
    ...
    d.updated_at
FROM deliveries d
INNER JOIN volunteers v ON v.id = d.volunteer_id
INNER JOIN food_requests fr ON fr.id = d.request_id
INNER JOIN ngos n ON n.id = fr.ngo_id
INNER JOIN food_donations fd ON fd.id = fr.donation_id
INNER JOIN donors donor ON donor.id = fd.donor_id
SQL;
    }
...
BEGIN
    ...
    IF v_owner_id IS NULL OR v_owner_id <> p_volunteer_id THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'This delivery is not assigned to the authenticated volunteer.';
    END IF;
    ...
    UPDATE deliveries
       SET delivery_status = p_new_status,
           pickup_time = CASE
               WHEN p_new_status = 'picked_up' AND pickup_time IS NULL THEN CURRENT_TIMESTAMP
               ELSE pickup_time
           END,
           delivered_at = CASE
               WHEN p_new_status = 'delivered' THEN CURRENT_TIMESTAMP
               ELSE delivered_at
           END,
    ...
    UPDATE food_requests
       SET request_status = CASE
               WHEN p_new_status = 'delivered' THEN 'completed'
               ELSE 'in_progress'
           END,
           updated_at = CURRENT_TIMESTAMP
    ...
    UPDATE volunteers
       SET availability_status = CASE
               WHEN p_new_status = 'delivered' THEN 'Available'
               ELSE 'Busy'
           END,
           updated_at = CURRENT_TIMESTAMP
...
SQL;
    ...
    private function triggerSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private function createSqliteTestView(): void
    {
        DB::statement('DROP VIEW IF EXISTS volunteer_delivery_details');
        DB::statement(<<<'SQL'
